...
```
The -iname option was used which means we can search for a specific pattern ("TXT" in this example) insensitively (upper case and lower case does not make a difference). This option is useful when we want to search for a specifc pattern when there's both upper case and lower case versions of the word.\
...
```
cd written_2
find written_2 -user jonathan > lab5.txt
```
```non-fiction
non-fiction/OUP
...
```
cd written_2
find travel_guides -user jonathan > lab5.txt
```
```
travel_guides
...
**Example 5**
```
find written_2 -type d > lab5.txt
```
```
written_2
